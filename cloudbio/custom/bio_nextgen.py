...
def _cufflinks_dev_configure_make(env):
    orig_eigen = "%s/include/eigen3" % env.system_install
    need_eigen = "%s/include/eigen3/include" % env.system_install
    if not exists(need_eigen):
        env.safe_sudo("ln -s %s %s" % (orig_eigen, need_eigen))
    run("./configure --program-suffix='-dev' --disable-werror --prefix=%s --with-eigen=%s" \
        " --with-boost=%s/boost" % (env.system_install, orig_eigen, env.system_install))
    run("make")
    env.safe_sudo("make install")
...
@_if_not_installed("tophat-dev")
def install_tophat_dev(env):
    _install_samtools_libs(env)
    _install_boost(env)
    version = "2.0.7"
    url = "http://tophat.cbcb.umd.edu/downloads/tophat-%s.tar.gz" % version
    _get_install(url, env, _cufflinks_dev_configure_make)

@_if_not_installed("cufflinks-dev")
def install_cufflinks_dev(env):
    _install_samtools_libs(env)
    _install_boost(env)
    version = "2.0.0"
    url = "http://cufflinks.cbcb.umd.edu/downloads/cufflinks-%s.tar.gz" % version
...
@_if_not_installed("jellyfish")
def install_jellyfish(env):
    version = "1.1.6"
    url = "http://www.cbcb.umd.edu/software/jellyfish/jellyfish-%s.tar.gz" % version
    _get_install(url, env, _configure_make)

@_if_not_installed("trimFastqReads.pl")
def install_curtain(env):
    version = "0.2.1"
    url = "http://curtain.googlecode.com/files/curtain-exec_%s-dist.tar.gz" % version
    _get_install(url, env, _make_copy("ls -1 bin/*", do_make=False))

#@_if_not_java_jar("gnx.jar")
def install_gnx(env):
    repository = "git clone git://github.com/mh11/gnx-tools.git"
    def _gnx_post_unpack_fn(env):
        run("mkdir bin")
        run("javac -d bin/ src/uk/ac/ebi/gnx/*")
        run("ant -f package.xml")
        run("echo '#!/bin/sh\njava -jar /usr/bin/gnx.jar' > gnx")
        run("chmod +x gnx")

    _get_install(repository, env, _make_copy("ls -1 gnx*", do_make=False), post_unpack_fn=_gnx_post_unpack_fn)
...
def install_abyss(env):
    ...
    def _remove_werror_get_boost(env):
        ...
        run("wget http://downloads.sourceforge.net/project/boost/boost/1.49.0/boost_1_49_0.tar.bz2")
        run("tar jxf boost_1_49_0.tar.bz2")
        run("ln -s boost_1_49_0/boost boost")
